...
const InformationRetrieval = () => {
    ...
    const [sectionValues, setSectionValues] = useState(['title','abstract']);
    ...
    const formSubmissionHandler = async (event) => {
        ...
        const url = `http://127.0.0.1:8000/api/v1/article/query_retrieval/?method=${method}&query=${query}&k=${k}&expansion=${checkboxIsChecked}&section=${section}`;
        setIsFormSubmitted(true);
        const resultData = await fetchData(url);
        setData(resultData);
    };

    const SectionValuesHandler = (event) => {
        const method = methodInputRef.current.value;
        setSectionValues(['abstract']);
        if (method == 'boolean'){
            setSectionValues(['title','authors'])
        }
        else if(method == 'elastic'){
            setSectionValues(['title','authors','abstract'])
        }
        else if(method == 'tfidf'){
            setSectionValues(['title','abstract'])
        }
    };
        ...
        <>
            <div className="form_container">
                <form onSubmit={formSubmissionHandler}>
                    ...
                    <label>Method</label>
                    <select ref={methodInputRef} id="method" name="method" onClick={SectionValuesHandler}>
                    ...
                    <select ref={sectionInputRef} id="section" name="section">
                        {(() => {
                            let rows = [];
                            for (let i = 0; i < sectionValues.length; i++) {
                            rows.push(<option value={sectionValues[i]} key={i}> {sectionValues[i]}</option>);
                            }
                            return rows;
                        })()};
                    </select>
            ...
            <div className='cards_container'>
                {isFormSubmitted && data.map((item, index) => (
                    <Card key={index} title={item.title} url={item.url} score={item.score}/>
                ))}
            </div>
        </>
    )
