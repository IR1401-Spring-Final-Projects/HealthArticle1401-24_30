...
const Clustering = () => {
    ...
    const formSubmissionHandler = async (event) => {
        ...
        const url = `http://127.0.0.1:8000/api/v1/article/clustering/?query=${query}&k=${k}`;
    ...
    return (
        <>
            ...
            <div className="cards_container">
                {isFormSubmitted && data.map((item, index) => <Card key={index} title={item.title} url={item.url} score={item.score} label={item.label}/>)}
            </div>
        </>
    )
